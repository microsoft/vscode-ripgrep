...
// Publishing with npm 5 does not work. See https://github.com/roblourens/vscode-ripgrep/issues/7
const npmPath = process.env['npm_execpath'];
const out = cp.execSync(`${npmPath} --version`).toString();
if (compareSemver(out, '5.0.0') >= 0) {
    throw new Error('NPM 5 is required for publishing vscode-ripgrep')
}
