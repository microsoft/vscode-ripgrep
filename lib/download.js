...
function downloadWin(url, dest, opts) {
    return new Promise((resolve, reject) => {
        ...
        let iwrCmd = `powershell Invoke-WebRequest -URI ${url} -UseBasicParsing -OutFile ${dest} -Headers ${headers}`;
        if (userAgent) {
...
function unzipWindows(zipPath, destinationDir) {
    return new Promise((resolve, reject) => {
        const expandCmd = 'powershell -Command Expand-Archive ' + ['-Path', zipPath, '-DestinationPath', destinationDir, '-Force'].join(' ');
